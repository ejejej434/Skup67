...
-- ============================================================================
local function decode_inventory_payload(raw)
    ...
    if ok and type(decoded) == "table" then
        return decoded
    return nil
local function normalize_webview_inventory(items)
    local normalized = {}
    for idx, item in ipairs(items or {}) do
        if type(item) == "table" then
            table.insert(normalized, {
                slot = tonumber(item.slot) or (idx - 1),
                name = tostring(item.name or item.itemName or "Unknown"),
                item_type = tonumber(item.type or item.itemType or 1) or 1,
                count = tonumber(item.count or item.amount or 1) or 1,
                item_id = tonumber(item.itemId or item.id or 0) or 0
            })
    return normalized
local function fetchInventoryFromWebView()
    if not webviews_status or not lib or type(lib.getJSValue) ~= "function" then
        return false
    end
    ...
    local js = [[(function () {
        ...
        if (window.inventory && Array.isArray(window.inventory.items)) {
            src = window.inventory.items;
        } else if (window.App && window.App.inventory) {
            if (Array.isArray(window.App.inventory.items)) src = window.App.inventory.items;
            else if (Array.isArray(window.App.inventory)) src = window.App.inventory;
        } else if (window.app && window.app.inventory && Array.isArray(window.app.inventory.items)) {
            src = window.app.inventory.items;
        }

        if (!src) return "[]";

        var mapped = src.map(function (item, index) {
            return {
                slot: item.slot ?? item.index ?? index,
                name: item.name ?? item.title ?? item.itemName ?? "Unknown",
                type: item.type ?? item.itemType ?? 1,
                count: item.count ?? item.amount ?? item.qty ?? 1,
    ...
    local ok, raw = pcall(lib.getJSValue, WEBVIEW_ID, js)
    if not ok then return false end

    local decoded = decode_inventory_payload(raw)
    if type(decoded) ~= "table" then return false end

    session_inv = normalize_webview_inventory(decoded)
    return true
if webviews_status and lib then
    function lib.onAction(data)
        if type(data) == "table" and data.type == "WV_LOADED" and tonumber(data.id) == WEBVIEW_ID then
            STATE.webviewReady = true

        local timeout = 40
        while not STATE.webviewReady and timeout > 0 do

        local ok = fetchInventoryFromWebView()
        STATE.waitingForInventory = false
        if not ok then
            addLog("Inventory parse via WebView failed")
        end
            return item.name == expectedName
local function executeSellViaWebView(slot, price, amount)
    if not webviews_status or not lib or type(lib.executeJS) ~= "function" then
        return false
    end
    ...
    local js = string.format([[(() => {
        ...
        const selectors = [
            `[data-slot="${slot}"]`,
            `[slot-id="${slot}"]`,
            `.inventory-slot[data-index="${slot}"]`,
            `.slot-${slot}`
        ];
        ...
        if (window.inventory && typeof window.inventory.sellItem === 'function') {
            window.inventory.sellItem(slot, price, amount);
            return true;
        }
        if (window.App && window.App.inventory && typeof window.App.inventory.sellItem === 'function') {
            window.App.inventory.sellItem(slot, price, amount);
            return true;
        }
        return false;
    })();]], slotNum, priceNum, amountNum)

    return pcall(lib.executeJS, WEBVIEW_ID, js)


        for _, item in ipairs(sell_queue) do

                addLog(" " .. item.slot .. "  , ")

            local ok = executeSellViaWebView(item.slot, item.price, item.amount)
            if not ok then
                addLog("WebView      " .. item.slot)
...
imgui.OnFrame(function() return UI.CentralGlMenu[0] or show_screen_btn[0] or UI.show_custom_lavka[0] end, function()
    ...
    if UI.CentralGlMenu[0] then 
        ...
        if not AUTH.isAuthorized then
            ...
            local nav_items = {
            imgui.TextDisabled("LMMR  Rodina Mobile")
                {u8"Ïðîäàæà", 6},
                {u8"Ëîãè", 2},
                {u8"Èíôî", 3},
                {u8"Êîíôèãè", 5}
